...
//#include "TFT_22_ILI9225.h"                                                       //подключаем библиотеку для работы с ЖКИ на ILI9225
...
//************************* Назначаем переменные ***********************************
  int position=1;                                                                 //переменная счетчика позиции меню
  int View_scale,Scale_low,Scale_hi,PWRround,PWRstep,Tone;                        //переменные вид шкалы, шкала малой мощности, большой мощности, округления, шаг округления и тон
  float SWRlow,SWRhi;                                                             //переменные КСВ
  int a, a_old, b_old,PWRthreshold;                                               //переменные для градусников прямой волны и КСВ
  float PWR,SWR,V,t;                                                              //переменные для изм. мощности, КСВ, напряжения, температуры
  float l,k;                                                                      //коэф. для измерения мощности 
  int flag_power=0;                                                               //флаг переключения низкой или высокой мощности
  int flag_clock=1, LCD=1;                                                        //флаг режима измерения, режим однократной очистки ЖКИ
  ...
  long powerTime=100;                                                             //время задержки возврата на малую мощность (около 10 секунд на Mega2560)
  unsigned long clockMillis=0;                                                    //начальное значение времени для счетчика clockMillis
  int clockTime=600;                                                              //время задержки включения заставки часов
  int val_clock=0;                                                                //задаем переменную, для счетчика часов
  int flag_scale=1;                                                               //переменная флага состояния шкалы мощности
  unsigned long dsLastUpdateTime=0;                                               //переменная счетчика датчика температуры
  bool flagT=0;                                                                   //флаг состояния вентилятора обдува
...
void swr(float U1,float U2){                                                      //подпрограмма КСВ с привязкой по U1 и U2
  ...
  if (SWR>=9.99){SWR=9.99;}                                                       //ограничиваем значение КСВ на уровне 9.99
    char bufSWR[3]; byte lenSWR=3;                                                //назначаем символьный массив, длина 3 символа
    dtostrf(SWR,lenSWR,2,bufSWR);                                                 //преобразуем в строку, точность отсчета до 2 знака
  if (SWR<2) {tft.drawText(58,8,bufSWR+String (' '),COLOR_GREEN);}                //выводим значение КСВ<2 зеленым цветом
  if ((SWR>=2)&&(SWR<3)){tft.drawText(58,8,bufSWR+String (' '),COLOR_ORANGE);}    //выводим значение КСВ=2...3 оранжевым цветом
  if (SWR>=3) {tft.drawText(58,8,bufSWR+String (' '),COLOR_RED);}                 //выводим значение КСВ>=3 красным цветом
  if ((SWR>=SWRhi)&&(flag_beep==1)){                                              //если КСВ больше верхнего порога включения и флаг сигнализации включен
  if (millis()-beepMillis>=1){beepMillis=millis();valBeep++; tone1.tone(2000);}}  //начинаем считать время, увеличиваем счетчик valBeep, включаем тональный сигнал
  else {valBeep=0; tone1.noTone();}                                               //иначе обнуляем счетчик и отключаем сигнал
  if (valBeep>=TimeBeep){valBeep=0; flag_beep=0; tone1.noTone();}                 //как только счетчик valBeep достигнет значения TimeBeep, обнуляем счетчик, флаг, откл. сигнал
  if (SWR<SWRlow){flag_beep=1; tone1.noTone();}                                   //если КСВ меньше нижнего порога, взводим флаг сигнализации и отключаем сигнал

    int b=map(SWR*10,10,30,0,20);                                                 //расчет переменной b (кубиков в градуснике)
  if (b>=20){b=20;}                                                               //ограничиваем градусник до КСВ=3.0
  if (View_scale==0){                                                             //если выбран вид шкалы с подложкой
  for(int i=0; i<b; i++){                                                         //включаем цикл рисования кубиков
  if (i<10){R=0;}                                                                 //определение красного цвета в градуснике
  if ((i>=10)&&(i<=15)){R=map(i,10,15,0,255);}                                    //---------------------------------------
  if (i>15){R=255;}                                                               //---------------------------------------

  if (i<=5){G=map(i,0,5,0,255);}                                                  //определение зеленого цвета в градуснике
  if ((i>5)&&(i<=15)){G=255;}                                                     //---------------------------------------
  if (i>15){G=map(i,16,19,255,0);}                                                //---------------------------------------

  if (i<=5) {B=255;}                                                              //определение синего цвета в градуснике
  if (i>9) {B=0;}                                                                 //-------------------------------------
  if ((i>5)&&(i<=9)){B=map(i,9,5,0,255);}                                         //-------------------------------------
    tft.fillRectangle(10+i*10,70,17+i*10,85,tft.setColor(R,G,B));}                //рисуем градиентный градусник пропорционально КСВ
  for(int j=b; j<20; j++){                                                        //включаем цикл рисования кубиков подложки и стирания
    tft.fillRectangle(10+j*10,70,17+j*10,85,tft.setColor(50,50,50));}}            //дорисовываем серую подложку до конца
  
  if (View_scale==1){                                                             //если выбран вид шкалы без подложки
  if (b > b_old) {                                                                //если новое значение больше старого(движение градусника вправо)
  for(int i=b_old; i<b; i++){                                                     //включаем цикл
  if (i<10){R=0;}                                                                 //определение красного цвета в градуснике
  if ((i>=10)&&(i<=15)){R=map(i,10,15,0,255);}                                    //---------------------------------------
  if (i>15){R=255;}                                                               //---------------------------------------
  ...
  if ((i>5)&&(i<=9)){B=map(i,9,5,0,255);}                                         //-------------------------------------
    tft.fillRectangle(10+i*10,70,17+i*10,85,tft.setColor(R,G,B));}                //рисуем градиентный градусник пропорционально КСВ
    b_old=b;}                                                                     //выравниваем старое и новое значение
  else {                                                                          //иначе
    tft.fillRectangle(10+b*10,70,17+b_old*10,85,COLOR_BLACK);                     //стираем градусник при движении влево
    b_old=b;}}                                                                    //выравниваем старое и новое значение
  }
...
//************************* Управление возвратом мощности ***************************
  if(PWR<PWRthreshold&&flag_power==1){                                            //если мощность меньше порога PWRthreshold и флаг режима мощности 1
  if(millis() - powerMillis >= 1){                                                //начинаем считать время
    powerMillis = millis();                                                       //выравниваем значения времени
    val_power++;}}                                                                //с каждой миллисекундой увеличиваем значение val
  else {val_power = 0;}                                                           //иначе обнуляем счётчик
  if (val_power >= powerTime){                                                    //как только значение val_power становится равным powerTime  
    flag_power=0; flag_scale=1;                                                   //сбрасываем флаг режима мощности в 0, флаг шкалы в 1
    val_power = 0;}                                                               //обнуляем счётчик
//************************* "Градусник" прямой волны  *******************************
  if (flag_power==0){                                                             //выбираем масштаб градусника входной мощности, если флаг равен 0
  if (Scale_low==0){a=PWR*2;}                                                     //для 20Вт
  if (Scale_low==1){a=PWR;}                                                       //для 40Вт
  if (Scale_low==2){a=PWR/2;}                                                     //для 80Вт
  if (Scale_low==3){a=PWR/3;}}                                                    //для 120Вт

  if (flag_power==1){                                                             //выбираем масштаб градусника выходной мощности, если флаг равен 1
  if (Scale_hi==0){a=PWR/5;}                                                      //для 200Вт
  if (Scale_hi==1){a=PWR/10;}                                                     //для 400Вт
  if (Scale_hi==2){a=PWR/15;}                                                     //для 600Вт
  if (Scale_hi==3){a=PWR/20;}                                                     //для 800Вт
  if (Scale_hi==4){a=PWR/25;}}                                                    //для 1000Вт
  
  if (View_scale==0){                                                             //если выбран вид шкалы с подложкой
  for (int i=0; i<a; i++){                                                        //включаем цикл рисования градусника
  
  if (i<20){R=0;}                                                                 //определение красного цвета в градуснике
  if ((i>=20)&&(i<=29)){R=map(i,20,29,0,255);}                                    //---------------------------------------
  if (i>29){R=255;}                                                               //---------------------------------------

  if (i<=9){G=map(i,0,9,0,255);}                                                  //определение зеленого цвета в градуснике
  if ((i>9)&&(i<=29)){G=255;}                                                     //---------------------------------------
  if (i>29){G=map(i,30,39,255,0);}                                                //---------------------------------------

  if (i<=9) {B=255;}                                                              //определение синего цвета в градуснике
  if (i>19) {B=0;}                                                                //-------------------------------------
  if ((i>9)&&(i<=19)){B=map(i,19,9,0,255);}                                       //-------------------------------------
    
    tft.fillRectangle(10+i*5,93,12+i*5,108,tft.setColor(R,G,B));}                 //рисуем градиентный градусник пропорционально мощности
  for(int j=a; j<40; j++){                                                        //включаем цикл рисования кубиков подложки и стирания
    tft.fillRectangle(10+j*5,93,12+j*5,108,tft.setColor(50,50,50));}}             //дорисовываем серую подложку до конца шкалы
  
  if (View_scale==1){                                                             //если выбран вид шкалы без подложки
  if (a > a_old) {                                                                //если новое значение a больше старого a_old (движение градусника вправо)
...
//************************************* Температура *************************************************
void temp(){byte data[0]; char buf[3]; byte len=4;                                  //назначаем переменные
    ds.reset(); ds.write(0xCC,1); ds.write(0x44,1);                                     //запрос темепературы с датчика
  if (millis()-dsLastUpdateTime>DS_UPDATE_TIME){                                    //каждое время DS_UPDATE_TIME 
    dsLastUpdateTime=millis();                                                      //
    ds.reset(); ds.write(0xCC,1); ds.write(0xBE,1);                                     //читаем данные с датчика
    data[0] = ds.read (); data[1] = ds.read ();                                     //в двух байтах
    int16_t raw = (data[1] << 8) | data[0];                                         //преобразуем в число
    t = (float) raw / 16.0;}                                                        //получаем температуру в градусах Цельсия
   
  if (t!=0){
    dtostrf(t, len, 1, buf);                                                        //преобразуем значение t в массив buf
    buf[len] = 'C'; buf[len+1] = 0;                                                 //добавляем к массиву знак C
    tft.drawText(35,155,buf+String (' '),COLOR_GREEN);                              //выводим значение температуры с пробелом в конце

  if (t>=TFanOn){digitalWrite(FanPin,HIGH); flagT=1; tft.drawText(120,155,"FAN ON  ", COLOR_RED);}
  if (t<=TFanOff){digitalWrite(FanPin,LOW); flagT=0; tft.drawText(120,155,"FAN OFF", COLOR_GREEN);}

  if ((t>TFanOff)&&(t<TFanOn)){
  if (flagT==1){tft.drawText(120,155,"FAN ON  ", COLOR_RED);}
  if (flagT==0){tft.drawText(120,155,"FAN OFF", COLOR_GREEN);}}}

  else {tft.drawText(3,155,"SWR & PWR-Meter D0ISM", COLOR_SKYBLUE);}
}
//****************************** Подпрограмма меню *****************************************
